...
Scope {
    ...
    property var blobAnimationEasing: Easing.InOutCirc
    // ============================================================
    // SIGNAL API
    // ============================================================

    signal hoveredBLBlob(); onHoveredBLBlob: if(!root.isBlobsDisabled) blBlobContent.enterHover()
    signal hoveredBCBlob(); onHoveredBCBlob: if(!root.isBlobsDisabled) bcBlobContent.enterHover()
    signal hoveredBRBlob(); onHoveredBRBlob: if(!root.isBlobsDisabled) brBlobContent.enterHover()
    signal hoveredTLBlob(); onHoveredTLBlob: if(!root.isBlobsDisabled) tlBlobContent.enterHover()
    signal hoveredTCBlob(); onHoveredTCBlob: if(!root.isBlobsDisabled) tcBlobContent.enterHover()
    signal hoveredTRBlob(); onHoveredTRBlob: if(!root.isBlobsDisabled) trBlobContent.enterHover()
    signal hoveredLEFTBlob(); onHoveredLEFTBlob: if(!root.isBlobsDisabled) leftBlobContent.enterHover()
    signal hoveredRIGHTBlob(); onHoveredRIGHTBlob: if(!root.isBlobsDisabled) rightBlobContent.enterHover()
    
    signal exitedBLBlob(); onExitedBLBlob: blBlobContent.leaveHover()
    signal exitedBCBlob(); onExitedBCBlob: bcBlobContent.leaveHover()
    signal exitedBRBlob(); onExitedBRBlob: brBlobContent.leaveHover()
    signal exitedTLBlob(); onExitedTLBlob: tlBlobContent.leaveHover()
    signal exitedTCBlob(); onExitedTCBlob: tcBlobContent.leaveHover()
    signal exitedTRBlob(); onExitedTRBlob: trBlobContent.leaveHover()
    signal exitedLEFTBlob(); onExitedLEFTBlob: leftBlobContent.leaveHover()
    signal exitedRIGHTBlob(); onExitedRIGHTBlob: rightBlobContent.leaveHover()

    // ============================================================
    // BLOB STATE
    // ============================================================
    property bool isBlobsDisabled:false
    
    property real blBlobProgress: ! root.isBlobsDisabled & (root.isBlBlobActive | root.isBlBlobAlwaysActive) ? 1 : 0
    property real bcBlobProgress: ! root.isBlobsDisabled & (root.isBcBlobActive | root.isBcBlobAlwaysActive) ? 1 : 0
    property real brBlobProgress: ! root.isBlobsDisabled & (root.isBrBlobActive | root.isBrBlobAlwaysActive) ? 1 : 0
    property real trBlobProgress: ! root.isBlobsDisabled & (root.isTrBlobActive | root.isTrBlobAlwaysActive) ? 1 : 0
    property real tcBlobProgress: ! root.isBlobsDisabled & (root.isTcBlobActive | root.isTcBlobAlwaysActive) ? 1 : 0
    property real tlBlobProgress: ! root.isBlobsDisabled & (root.isTlBlobActive | root.isTlBlobAlwaysActive) ? 1 : 0
    property real leftBlobProgress: ! root.isBlobsDisabled & (root.isLeftBlobActive | root.isLeftBlobAlwaysActive) ? 1 : 0
    property real rightBlobProgress: ! root.isBlobsDisabled & (root.isRightBlobActive | root.isRightBlobAlwaysActive) ? 1 : 0

    property bool isBrBlobAlwaysActive: false
    property bool isBcBlobAlwaysActive: false
    property bool isBlBlobAlwaysActive: false
    property bool isTrBlobAlwaysActive: false
    property bool isTcBlobAlwaysActive: false
    property bool isTlBlobAlwaysActive: false
    property bool isLeftBlobAlwaysActive: false
    property bool isRightBlobAlwaysActive: false
    
    readonly property bool isBrBlobOpened: brBlobContent.isBlobFocused
    readonly property bool isBcBlobOpened: bcBlobContent.isBlobFocused
    readonly property bool isBlBlobOpened: blBlobContent.isBlobFocused
    readonly property bool isTrBlobOpened: trBlobContent.isBlobFocused
    readonly property bool isTcBlobOpened: tcBlobContent.isBlobFocused
    readonly property bool isTlBlobOpened: tlBlobContent.isBlobFocused
    readonly property bool isLeftBlobOpened: leftBlobContent.isBlobFocused
    readonly property bool isRightBlobOpened: rightBlobContent.isBlobFocused
    
    readonly property bool isBrBlobActive:
        brBlobContent.isBlobFocused || brBlobContent.hovered
    
    readonly property bool isBcBlobActive:
        bcBlobContent.isBlobFocused || bcBlobContent.hovered
    
    readonly property bool isBlBlobActive:
        blBlobContent.isBlobFocused || blBlobContent.hovered
    
    readonly property bool isTrBlobActive:
        trBlobContent.isBlobFocused || trBlobContent.hovered
    
    readonly property bool isTcBlobActive:
        tcBlobContent.isBlobFocused || tcBlobContent.hovered
    
    readonly property bool isTlBlobActive:
        tlBlobContent.isBlobFocused || tlBlobContent.hovered
    
    readonly property bool isLeftBlobActive:
        leftBlobContent.isBlobFocused || leftBlobContent.hovered
    
    readonly property bool isRightBlobActive:
        rightBlobContent.isBlobFocused || rightBlobContent.hovered
    
    readonly property bool anyBlobModuleSelected:
        blBlobContent.activeModule !== "" ||
        bcBlobContent.activeModule !== "" ||
        brBlobContent.activeModule !== "" ||
        tlBlobContent.activeModule !== "" ||
        tcBlobContent.activeModule !== "" ||
        trBlobContent.activeModule !== "" ||
        leftBlobContent.activeModule !== "" ||
        rightBlobContent.activeModule !== ""
    ...
    // ==============     S I N G L E T O N S     =================
    // ============================================================
    property var focusedBlob: null

    Item {
        id: focusManager

        property var focusedBlob: root.focusedBlob

        onFocusedBlobChanged: {
            root.focusedBlob = focusedBlob
            console.log("[FocusManager] focused blob:", focusedBlob ? focusedBlob.objectName : "none")
        }
    }

    ModuleRegistry {
        id: moduleRegistry
        searchPaths: [Quickshell.shellPath("desktop/modules")]
    }
    ...
    BlobIPCHandler {
        ...
        blobs: ({
            ...
            bl: blBlobContent,
            bc: bcBlobContent,
            left: leftBlobContent,
            right: rightBlobContent
        })
    }
    ...
    PanelWindow {
        ...
        WlrLayershell.namespace: "n0ctos-desktop-frame"
        WlrLayershell.keyboardFocus:
            root.anyBlobModuleSelected ||
            focusManager.focusedBlob !== null
        ...
        // HOVER MOUSE AREAS
        // ========================================================
        // blob outside dismiss area
        MouseArea {
            id: outsideClickArea
            Rectangle{color:"#3f009fff";visible:true;anchors.fill:parent}
        
            anchors.fill: parent
            z: -100
        
            visible: root.anyBlobModuleSelected
            enabled: root.anyBlobModuleSelected
            acceptedButtons: Qt.LeftButton
        
            onPressed: function(mouse) {
                mouse.accepted = true
        
                const blob = focusManager.focusedBlob
        
                if (blob && blob.isBlobFocused)
                    blob.unfocusBlob()
            }
        }
        // bottom left
        ...
        Blob {
            ...
            moduleIds: blobManager.blobs.bl
            focusManager: focusManager
            visible: root.isBlBlobActive || root.blBlobProgress > 0
            opacity: root.blBlobProgress
            scale: root.blBlobProgress
            y: frameWindow.height - root.frameSide("bottom") - height
            x: root.frameSide("left")
            height: implicitHeight * root.blBlobProgress
            width: implicitWidth * root.blBlobProgress
        }
        // bottom right
        Blob {
            id: brBlobContent
            ...
            blobId: "br"
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.br
            focusManager: focusManager
            visible: root.isBrBlobActive || root.brBlobProgress > 0
            opacity: root.brBlobProgress
            scale: root.brBlobProgress
            y: frameWindow.height - root.frameSide("bottom") - height
            x: frameWindow.width - width - root.frameSide("right")
            height: implicitHeight * root.brBlobProgress
            width: implicitWidth * root.brBlobProgress
        }
        // bottom center
        Blob {
            id: bcBlobContent
            ...
            blobId: "bc"
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.bc
            focusManager: focusManager
            visible: root.isBcBlobActive || root.bcBlobProgress > 0
            opacity: root.bcBlobProgress
            scale: root.bcBlobProgress
            y: frameWindow.height - root.frameSide("bottom") - height
            x: root.innerGeometry().x + (root.innerGeometry().width - width) * 0.5
            height: implicitHeight * root.bcBlobProgress
            width: implicitWidth * root.bcBlobProgress
        }
        // top left
        Blob {
            id: tlBlobContent
            ...
            blobId: "tl"
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.tl
            focusManager: focusManager
            y: root.frameSide("top")
            x: root.frameSide("left")
            height: implicitHeight * root.tlBlobProgress
            width: implicitWidth * root.tlBlobProgress
            visible: root.isTlBlobActive || root.tlBlobProgress > 0
            opacity: root.tlBlobProgress
            scale: root.tlBlobProgress
        }
        // top right
        Blob {
            id: trBlobContent
            ...
            blobId: "tr"
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.tr
            focusManager: focusManager
            visible: root.isTrBlobActive || root.trBlobProgress > 0
            opacity: root.trBlobProgress
            scale: root.trBlobProgress
            y: root.frameSide("top")
            x: frameWindow.width - width - root.frameSide("right")
            height: implicitHeight * root.trBlobProgress
            width: implicitWidth * root.trBlobProgress
        }
        // top center
        Blob {
            id: tcBlobContent
            ...
            blobId: "tc"
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.tc
            focusManager: focusManager
            visible: root.isTcBlobActive || root.tcBlobProgress > 0
            opacity: root.tcBlobProgress
            scale: root.tcBlobProgress
            y: root.frameSide("top")
            x: root.innerGeometry().x + (root.innerGeometry().width - width) * 0.5
            height: implicitHeight * root.tcBlobProgress
            width: implicitWidth * root.tcBlobProgress
          }
        // left
        Blob {
            id: leftBlobContent
            ...
            blobId: "left"
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.left
            focusManager: focusManager
            visible: root.isLeftBlobActive || root.leftBlobProgress > 0
            opacity: root.leftBlobProgress
            scale: root.leftBlobProgress
            x: root.frameSide("left")
            y: root.innerGeometry().y + (root.innerGeometry().height - height) * 0.5
            height: implicitHeight * root.leftBlobProgress
            width: implicitWidth * root.leftBlobProgress
        }
        // right
        Blob {
            id: rightBlobContent
            ...
            isVertical: true
            moduleRegistry: moduleRegistry
            moduleIds: blobManager.blobs.right
            focusManager: focusManager
            visible: root.isRightBlobActive || root.rightBlobProgress > 0
            opacity: root.rightBlobProgress
            scale: root.rightBlobProgress
            x: frameWindow.width - width - root.frameSide("right")
            y: root.innerGeometry().y + (root.innerGeometry().height - height) * 0.5
            height: implicitHeight * root.rightBlobProgress
            width: implicitWidth * root.rightBlobProgress
        }
        ...
        mask: Region{
          ...
          Region {
                  x: 0
                  y: 0
                  width: root.anyBlobModuleSelected ? outsideClickArea.width : 0
                  height: root.anyBlobModuleSelected ? outsideClickArea.height : 0
              }
        ...
        Component.onCompleted: {
            ...
            console.log("[Shell] module paths:", JSON.stringify(moduleRegistry.searchPaths))
            console.log("[Shell] config paths:", JSON.stringify(blobManager.configPaths))
            console.log("============================")
        }
    }
}
